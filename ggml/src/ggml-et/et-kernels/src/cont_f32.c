...
// - Each thread processes whole cachelines (64 bytes = 16 F32 elements)
...
int entry_point(struct ggml_et_cont_params* params, void* env) {
    ...
    const int64_t ELEMENTS_PER_CACHELINE = CACHELINE_SIZE / sizeof(float);  // 16 elements
    ...
    // Process this thread's portion of elements in destination order (sequential writes)
    for (int64_t dst_linear_idx = start_element; dst_linear_idx < end_element; dst_linear_idx++) {
        ...
        const float* src_ptr = (const float*)((char*)src0_data + src_offset_bytes);
