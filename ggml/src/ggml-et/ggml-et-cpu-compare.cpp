...
bool ggml_et_cpu_compare_init_pre(ggml_et_cpu_compare_ctx* ctx, const ggml_tensor* node, ggml_op op) {
    if (!ctx || !node) {
        ET_LOG_ERROR("ET: Invalid parameters for CPU compare init\n");
        return false;
    }
    ...
    auto get_tensor_buffer_size = [](const ggml_tensor* tensor) -> size_t {
        ...
        if (tensor->buffer) {
            ...
            ET_LOG_DEBUG("ET: Tensor buffer size: logical=%zu, backend_buffer=%zu, contiguous=%s\n",
    ...
    ET_LOG_DEBUG("ET: CPU compare init for operation %s\n", ggml_op_name(op));
    ET_LOG_DEBUG("ET: Tensor sizes - src0:%zu src1:%zu src2:%zu dst:%zu bytes\n",
    ...
    if (ctx->src0_size > 0) {
        ...
        if (!ctx->cpu_src0_data) {
            ET_LOG_ERROR("ET: Failed to allocate CPU src0 buffer\n");
            goto cleanup;
        }
    }
    ...
    if (ctx->src1_size > 0) {
        ...
        if (!ctx->cpu_src1_data) {
            ET_LOG_ERROR("ET: Failed to allocate CPU src1 buffer\n");
            goto cleanup;
        }
    }
    ...
    if (ctx->src2_size > 0) {
        ...
        if (!ctx->cpu_src2_data) {
            ET_LOG_ERROR("ET: Failed to allocate CPU src2 buffer\n");
            goto cleanup;
        }
    }
    ...
    if (!ctx->cpu_dst_data) {
        ET_LOG_ERROR("ET: Failed to allocate CPU dst buffer\n");
        goto cleanup;
    }

    ctx->et_dst_data = malloc(ctx->dst_size);
    if (!ctx->et_dst_data) {
        ET_LOG_ERROR("ET: Failed to allocate ET dst buffer\n");
        goto cleanup;
    }

    // Copy data from ET device buffers to CPU host buffers
    ET_LOG_DEBUG("ET: Copying data from ET device buffers to CPU host buffers\n");
    ...
    ET_LOG_DEBUG("ET: Creating CPU backend for reference computation\n");
    ctx->cpu_backend = ggml_backend_cpu_init();
    if (!ctx->cpu_backend) {
        ET_LOG_ERROR("ET: Failed to create CPU backend\n");
        goto cleanup;
    }

    // Create GGML context for CPU tensors
    ET_LOG_DEBUG("ET: Creating GGML context for CPU computation\n");
    ...
    if (!ctx->ggml_ctx) {
        ET_LOG_ERROR("ET: Failed to create GGML context\n");
        goto cleanup;
    }

    // Create CPU tensors with proper context
    ET_LOG_DEBUG("ET: Creating CPU tensors\n");

    if (node->src[0]) {
        ctx->cpu_src0 = ggml_new_tensor(ctx->ggml_ctx, node->src[0]->type, GGML_MAX_DIMS, node->src[0]->ne);
        if (!ctx->cpu_src0) {
            ET_LOG_ERROR("ET: Failed to create CPU src0 tensor\n");
            goto cleanup;
        }
    ...
    if (node->src[1]) {
        ...
        if (!ctx->cpu_src1) {
            ET_LOG_ERROR("ET: Failed to create CPU src1 tensor\n");
            goto cleanup;
        }
    ...
    if (node->src[2]) {
        ...
        if (!ctx->cpu_src2) {
            ET_LOG_ERROR("ET: Failed to create CPU src2 tensor\n");
            goto cleanup;
        }
...
bool ggml_et_cpu_compare_compute_and_check(ggml_et_cpu_compare_ctx* ctx, const ggml_tensor* node,
    ...
    if (!ctx || !ctx->cpu_backend || !ctx->ggml_ctx || !node || !config) {
        ET_LOG_ERROR("ET: Invalid parameters for CPU compute and check\n");
        return false;
    }
    ...
    switch (op) {
        ...
        case GGML_OP_GLU:
            // Extract GLU parameters from op_params (split mode only)
            {
                ...
                if (!ctx->cpu_src1) {
                    ET_LOG_ERROR("ET: GLU CPU comparison requires split tensor mode\n");
                    return false;
                }
        ...
        case GGML_OP_SET_ROWS:
            {
                ...
                if (!cpu_dst_base) {
                    ET_LOG_ERROR("ET: Failed to create CPU destination base tensor for SET_ROWS\n");
                    return false;
                }
            ...
            break;
        default:
            ET_LOG_ERROR("ET: Unsupported operation %s for CPU comparison\n", ggml_op_name(op));
            return false;
    }

    if (!ctx->cpu_dst) {
        ET_LOG_ERROR("ET: Failed to create CPU destination tensor for operation %s\n", ggml_op_name(op));
        return false;
    }
    ...
    ET_LOG_DEBUG("ET: Creating CPU computation graph\n");
    ctx->cpu_graph = ggml_new_graph_custom(ctx->ggml_ctx, 1, false);
    if (!ctx->cpu_graph) {
        ET_LOG_ERROR("ET: Failed to create CPU computation graph\n");
        return false;
    }
    ...
    if (config && config->log_differences) {
        if (ctx->cpu_src0_data && ctx->src0_size >= 4) {
            ET_LOG_DEBUG("ET: CPU src0 first few bytes: %02x %02x %02x %02x\n",
        ...
        if (ctx->cpu_src1_data && ctx->src1_size >= 16) {
            ET_LOG_DEBUG("ET: CPU src1 first few floats: %.6f %.6f %.6f %.6f\n",
    ...
    if (config->log_differences) {
        ET_LOG_DEBUG("ET: Comparing ET vs CPU results\n");
        ...
        ET_LOG_DEBUG("ET: First %zu elements comparison (checking all %zu elements, type=%s):\n",
        ...
        for (size_t i = 0; i < max_log; i++) {
            ...
            if (is_elementwise && cpu_src0_float && cpu_src1_float) {
                ET_LOG_DEBUG("ET: [%zu] src0=%.6f, src1=%.6f -> CPU=%.6f, ET=%.6f, diff=%.6f\n",
                              i, cpu_src0_float[i], cpu_src1_float[i], cpu_val, et_val, diff);
            } else if (is_elementwise && cpu_src0_float) {
                ET_LOG_DEBUG("ET: [%zu] src0=%.6f -> CPU=%.6f, ET=%.6f, diff=%.6f\n",
                              i, cpu_src0_float[i], cpu_val, et_val, diff);
            } else {
                ET_LOG_DEBUG("ET: [%zu] CPU=%.6f, ET=%.6f, diff=%.6f\n",
        ...
        if (num_elements > max_log) {
            ...
            ET_LOG_DEBUG("ET: Middle element [%zu]: CPU=%.6f, ET=%.6f\n",
                          mid, cpu_mid, et_mid);
            ET_LOG_DEBUG("ET: Last element [%zu]: CPU=%.6f, ET=%.6f\n",
                          end, cpu_end, et_end);
        }

        ET_LOG_DEBUG("ET: Results %s (%zu/%zu elements match within tolerance %.6f)\n",
    ...
    if (config->use_cpu_result) {
        ET_LOG_DEBUG("ET: Overwriting ET device result with CPU result for correct inference\n");
        size_t dst_logical_size = ggml_nbytes(node);
        ggml_backend_tensor_set(const_cast<ggml_tensor*>(node), ctx->cpu_dst_data, 0, dst_logical_size);
        ET_LOG_DEBUG("ET: CPU result copied to ET device buffer\n");
    }
